...
var dataset = _babel_runtime_corejs3_core_js_stable_instance_map__WEBPACK_IMPORTED_MODULE_0___default()(_context = d3.range(n)).call(_context, function (d, i) {
  var intercept = 40;
  var random = d3.randomNormal(0, 10)();
  var value = intercept + i + random;
  return value;
});
/* Styles ------------------------------------------------------------------ */
...
function drawGraphic(containerWidth) {
  ...
  var height = containerWidth * 0.7 - margin.top - margin.bottom;
  var xScale = d3.scaleLinear().domain([0, n]).range([0, width]);
  var yScale = d3.scaleLinear().domain([0, n * 1.6]).range([height, 0]);
  ...
  var vis = d3.select("#vis").append("svg").attr("id", "graphic").attr("width", width + margin.left + margin.right).attr("height", height + margin.top + margin.bottom).append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")");
  var xAxis = vis.append("g").attr("class", "axis xaxis").attr("transform", "translate(0," + height + ")").call(d3.axisBottom(xScale));
  var yAxis = vis.append("g").attr("class", "axis yaxis").call(d3.axisLeft(yScale));
  var path = vis.append("path").datum(dataset).attr("class", "line").attr("d", line);
}
/* Draw -------------------------------------------------------------------- */


function drawGraphicInContext() {
  var rect = document.getElementById("vis").getBoundingClientRect();
  drawGraphic(rect.width);
}
/* Main -------------------------------------------------------------------- */


window.addEventListener('resize', function () {
  return drawGraphicInContext();
});
addStyleSheet("style.css", function () {
  return drawGraphicInContext();
});
